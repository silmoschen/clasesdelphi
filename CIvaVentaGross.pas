...
begin
  if (tipolistado = 'P') or (tipolistado = 'I') then Begin
    ...
    list.Titulo(94, list.lineactual, 'Accesor.',6 , 'Arial, cursiva, 8');
    list.Titulo(103, list.lineactual, 'Honorarios',7 , 'Arial, cursiva, 8');
    list.Titulo(113, list.lineactual, 'Medicamen.',8 , 'Arial, cursiva, 8');
    list.Titulo(127, list.lineactual, 'I.V.A.',9 , 'Arial, cursiva, 8');
    list.Titulo(137, list.lineactual, 'Total',10 , 'Arial, cursiva, 8');
...
begin
  if (salida = 'P') or (salida = 'I') then Begin
   ...
   list.importe(101, list.lineactual, '', totOpexenta, 4, 'Arial, negrita, 8');
   list.importe(111, list.lineactual, '', totConnograv, 5, 'Arial, negrita, 8');
   list.importe(121, list.lineactual, '', totIva, 6, 'Arial, negrita, 8');
   list.importe(131, list.lineactual, '', totIvarec, 7, 'Arial, negrita, 8');
   list.importe(141, list.lineactual, '', totPercepcion, 8, 'Arial, negrita, 8');
  ...
  if salida = 'T' then Begin
   ...
   List.ImporteTxt(totPercepcion, 10, 2, True);
  end;
end;
...
begin
  if (salida = 'P') or (salida = 'I') then Begin
   if not infresumido then Begin
    ...
    list.importe(101,  list.lineactual, '', tiva.FieldByName('total3').AsFloat, 8, 'Arial, normal, 8');
    list.importe(111,  list.lineactual, '', tiva.FieldByName('total2').AsFloat, 9, 'Arial, normal, 8');
    list.importe(121,  list.lineactual, '', tiva.FieldByName('total1').AsFloat, 10, 'Arial, normal, 8');
    list.importe(131, list.lineactual, '', tiva.FieldByName('Iva').AsFloat, 11, 'Arial, normal, 8');
    list.importe(141, list.lineactual, '', tiva.FieldByName('Totoper').AsFloat, 13, 'Arial, normal, 8');
    list.Linea(142, list.lineactual, ' ', 14, 'Arial, normal, 8', salida, 'S');
   end;
  end else Begin
    ...
    list.ImporteTxt(tiva.FieldByName('total3').AsFloat, 9, 2, False);
    list.ImporteTxt(tiva.FieldByName('total2').AsFloat, 9, 2, False);
    list.ImporteTxt(tiva.FieldByName('total1').AsFloat, 9, 2, False);
    list.ImporteTxt(tiva.FieldByName('iva').AsFloat, 9, 2, False);
    list.ImporteTxt(tiva.FieldByName('totoper').AsFloat, 10, 2, True);
    Inc(lineasimpresas);
  end;
  ...
  if tiva.FieldByName('tipomov').AsString <> 'X' then Begin   // Si el comprobante no esta anulado
    ...
    totOpexenta   := totOpexenta   + tiva.FieldByName('total3').AsFloat;
    totConnograv  := totConnograv  + tiva.FieldByName('total2').AsFloat;
    totIva        := totIva        + tiva.FieldByName('total1').AsFloat;
    totIvarec     := totIvarec     + tiva.FieldByName('iva').AsFloat;
    totPercepcion := totPercepcion + tiva.FieldByName('totoper').AsFloat;
  end;
