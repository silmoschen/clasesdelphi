...
uses SysUtils, DB, DBTables, CBDT, CProvin, CListar, CUtiles, CIDBFM;
...
TTCodpost = class(TObject)            // Superclase
 ...
 public
  ...
  function    getLocalidades: TQuery;
...
begin
  inherited Create;
  tabla := datosdb.openDB('codpost', 'cp;orden');
end;
...
begin
  ...
  Result := datosdb.Buscar(tabla, 'cp', 'orden', xcp, xorden);
end;
...
begin
  if tabla.IndexFieldNames <> 'cp;orden' then tabla.IndexFieldNames := 'cp;orden';
  datosdb.BuscarEnFormaContextual(tabla, 'cp', 'orden', Copy(xexpr, 1, 4), Copy(xexpr, 5, 3));
end;
...
function TTCodpost.getLocalidades: TQuery;
begin
  result := datosdb.tranSQL('select * from ' + tabla.TableName + ' order by localidad');
end;
