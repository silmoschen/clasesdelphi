unit CIvaventaEspiga;

interface

uses CIva, CTablaIva, Cliengar, CCliente, CCNetos, SysUtils, DB, DBTables, CBDT,
     CUtiles, CListar, Listado, CEmpresas, CIDBFM, CUtilidadesArchivos, CCereales_Espiga,
     CServers2000_Excel;

type

TTivaventa = class(TTIva)
  idcereal: String; Precio, Kg: Real;
  anulado: Boolean;
 public
  ...
  procedure   Grabar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xclipro, xrsocial, xcodiva, xconcepto, xferecep, xfecha, xcodprovin, xcodmov, xtipomov: string;
                     xnettot, xopexenta, xconnograv, xiva, xivarec, xpercep1, xpercep2, xcdfiscal, xtotoper, xprecio, xkg: real; xcontrol, xidcereal: String); overload;
...
procedure TTIvaventa.Grabar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xclipro, xrsocial, xcodiva, xconcepto, xferecep, xfecha, xcodprovin, xcodmov, xtipomov: string;
                     xnettot, xopexenta, xconnograv, xiva, xivarec, xpercep1, xpercep2, xcdfiscal, xtotoper, xprecio, xkg: real; xcontrol, xidcereal: String);
// Objetivo...: Grabar Atributos del Objeto
begin
  if Buscar(xidcompr, xtipo, xsucursal, xnumero) then tiva.Edit else tiva.Append;
  inherited Act_atributos(xidcompr, xtipo, xsucursal, xnumero, xcuit, xclipro, xrsocial, xcodiva, xconcepto, xferecep, xfecha, xcodprovin, xcodmov, xnettot, xopexenta, xconnograv, xiva, xivarec, xpercep1, xpercep2, xcdfiscal, xtotoper);
  ...
  tiva.FieldByName('idcereal').AsString := xidcereal;
  tiva.FieldByName('precio').AsFloat    := xprecio;
  tiva.FieldByName('kg').AsFloat        := xkg;
  try
    tiva.Post;
   except
    tiva.Cancel;
...
begin
  ...
  if tiva.FieldByName('tipomov').AsString = 'X' then anulado := True else anulado := False;
  idcereal := tiva.FieldByName('idcereal').AsString;
  precio   := tiva.FieldByName('precio').AsFloat;
  kg       := tiva.FieldByName('kg').AsFloat;
end;
...
begin
  ...
  if tipolistado = 'T' then titulosTxt;
  if tipolistado = 'X' then Begin
    ...
    excel.setString('k' + ff, 'k' + ff, 'Percep.', 'Arial, normal, 8');
    excel.Alinear('l' + ff, 'l' + ff, 'D');
    excel.setString('l' + ff, 'l' + ff, 'Retenc.', 'Arial, normal, 8');
    excel.Alinear('m' + ff, 'm' + ff, 'D');
    excel.setString('m' + ff, 'm' + ff, 'Total', 'Arial, normal, 8');
    excel.Alinear('n' + ff, 'n' + ff, 'D');
    excel.setString('n' + ff, 'n' + ff, l1, 'Arial, normal, 8');
    excel.setString('o' + ff, 'o' + ff, 'Cereal', 'Arial, normal, 8');
    excel.Alinear('p' + ff, 'p' + ff, 'D');
    excel.setString('p' + ff, 'p' + ff, 'Precio', 'Arial, normal, 8');
    excel.Alinear('q' + ff, 'q' + ff, 'D');
    excel.setString('q' + ff, 'q' + ff, 'Kg', 'Arial, normal, 8');
    // 2º Línea de Títulos
    Inc(fila); ff := Trim(IntToStr(fila));
    excel.Alinear('g' + ff, 'g' + ff, 'D');
    ...
    excel.Alinear('k' + ff, 'k' + ff, 'D');
    excel.setString('k' + ff, 'k' + ff, 'Ing. Brutos', 'Arial, normal, 8');
    excel.Alinear('l' + ff, 'l' + ff, 'D');
    excel.setString('l' + ff, 'l' + ff, 'I.V.A.', 'Arial, normal, 8');
    excel.Alinear('m' + ff, 'm' + ff, 'D');
    excel.setString('m' + ff, 'm' + ff, 'Operación', 'Arial, normal, 8');
    excel.Alinear('n' + ff, 'n' + ff, 'D');
    excel.setString('n' + ff, 'n' + ff, l2, 'Arial, normal, 8');
    ...
    excel.FijarAnchoColumna('o' + ff, 'o' + ff, 16);
...
begin
  ...
  if salida = 'X' then Begin
    if Trim(leyenda) = 'Subtotales:' then Begin
      ...
      excel.setFormulaArray('p' + ff, 'p' + ff, '=suma(' + 'p' + IntToStr(ffi) + '..' + 'p' + IntToStr(f) + ')', 'Arial, negrita, 8');
      excel.setFormulaArray('q' + ff, 'q' + ff, '=suma(' + 'q' + IntToStr(ffi) + '..' + 'q' + IntToStr(f) + ')', 'Arial, negrita, 8');
...
begin
  ...
  if salida = 'T' then SaltoTxt(salida);

  if salida = 'X' then Begin
    Inc(fila); ff := Trim(IntToStr(fila));
    excel.Alinear('a' + ff, 'a' + ff, 'I');
    ...
    excel.setReal('k' + ff, 'k' + ff, tiva.FieldByName('Percep2').AsFloat, 'Arial, normal, 8');
    ...
    excel.setReal('n' + ff, 'n' + ff, tiva.FieldByName('Cdfiscal').AsFloat, 'Arial, normal, 8');
    cereales.getDatos(tiva.FieldByName('idcereal').AsString);
    excel.setString('o' + ff, 'o' + ff, cereales.Descrip, 'Arial, normal, 8');
    excel.setReal('p' + ff, 'p' + ff, tiva.FieldByName('precio').AsFloat, 'Arial, normal, 8');
    excel.setReal('q' + ff, 'q' + ff, tiva.FieldByName('kg').AsFloat, 'Arial, normal, 8');
  end;
end;
...
begin
  ...
  while not tiva.EOF do Begin
    ...
    if t_filtro = '6' then     // Filtro por Fecha de Recepción
      if (tiva.FieldByName('ferecep').AsString >= p1) and (tiva.FieldByName('ferecep').AsString <= p2) and (tiva.FieldByName('idcereal').AsString = p3) then LineaIva(salida);
    tiva.Next;
  end;
...
begin
  ...
  if not datosdb.verificarSiExisteCampo('ivaventa', 'idcereal', path) then datosdb.tranSQL(path, 'alter table ivaventa add idcereal char(3)');
  if not datosdb.verificarSiExisteCampo('ivaventa', 'precio', path) then datosdb.tranSQL(path, 'alter table ivaventa add precio numeric');
  if not datosdb.verificarSiExisteCampo('ivaventa', 'kg', path) then datosdb.tranSQL(path, 'alter table ivaventa add kg numeric');
