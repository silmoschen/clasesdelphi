...
begin
  if (dbs.BaseClientServ = 'N') and (Length(Trim(dbs.baseDat_N)) = 0) then bloqueos := datosdb.openDB('bloqueos', '');
  if (dbs.BaseClientServ = 'S') or (Length(Trim(dbs.baseDat_N)) > 0) then bloqueos := datosdb.openDB('bloqueos', '', '', dbs.baseDat_N);
end;
