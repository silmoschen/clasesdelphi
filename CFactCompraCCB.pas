...
uses CComprobantes, CProveedorCCB, SysUtils, DB, DBTables, CIDBFM,
     CUtiles, CBDT, CInsumos_CentroBioq;
...
begin
  ...
  tdetalle  := datosdb.openDB('detcompr',  '', '', directorio);
  tcabecera := datosdb.openDB('cabcomp', '', '', directorio);
end;
...
begin
  if BuscarCab(xidcompr, xtipo, xsucursal, xnumero, xidtitular) then
    begin
      datosdb.tranSQL(directorio, 'DELETE FROM ' + tdetalle.TableName + ' WHERE idcompr = ' + '''' + xidcompr + '''' + ' and tipo = ' + '''' + xtipo + '''' + ' and sucursal = ' + '''' + xsucursal + '''' + ' and numero = ' + '''' + xnumero + '''' + ' and idtitular = ' + '''' + xidtitular + '''');
      tcabecera.Delete;      // Cabecera
      datosdb.refrescar(tcabecera); datosdb.refrescar(tdetalle); 
    end;
...
begin
  Result := datosdb.tranSQL(directorio, 'SELECT * FROM detcompr WHERE idcompr = ' + '''' + xidcompr + '''' + ' and tipo = ' + '''' + xtipo + '''' + ' and sucursal = ' + '''' + xsucursal + '''' + ' and numero = ' + '''' + xnumero + '''' + ' and idtitular = ' + '''' + xidtitular + '''');
end;
...
begin
  TSQL := datosdb.tranSQL(directorio, 'SELECT cabcomp.fecha, cabcomp.idcompr, cabcomp.tipo, cabcomp.sucursal, cabcomp.numero, provedor.rsocial, cabcomp.subtotal, cabcomp.bonif, cabcomp.impuestos, cabcomp.ivari, cabcomp.ivarni, cabcomp.sobretasa ' +
  ...
  inherited llistFechas(df, hf, 'Informe de Compras por Fecha - Lapso: ' + df + '-' + hf, salida);  // Heredamos, el método se implementa para compas y ventas
end;

procedure TTFacturaCompra.listCliProv(df, hf: string; salida: char);
// Objetivo...: Generar informe de Ventas - Nivel de ruptura por fecha
begin
  TSQL := datosdb.tranSQL(directorio, 'SELECT cabcomp.fecha, cabcomp.idcompr, cabcomp.tipo, cabcomp.sucursal, cabcomp.numero, provedor.codprov AS clipro, provedor.rsocial, cabcomp.subtotal, cabcomp.bonif, cabcomp.impuestos, cabcomp.ivari, cabcomp.ivarni, cabcomp.sobretasa ' +
...
begin
  TSQL := datosdb.tranSQL(directorio, 'SELECT cabcomp.fecha, cabcomp.idcompr, cabcomp.tipo, cabcomp.sucursal, cabcomp.numero, provedor.codprov AS clipro, provedor.rsocial, provedoh.codpfis, cabcomp.subtotal, cabcomp.bonif, cabcomp.impuestos, cabcomp.ivari, cabcomp.ivarni, ' +
...
begin
  TSQL := datosdb.tranSQL(directorio, 'SELECT cabcomp.fecha, cabcomp.idcompr, cabcomp.tipo, cabcomp.sucursal, cabcomp.numero, cabcomp.cc, provedor.codprov AS clipro, provedor.rsocial, cabcomp.subtotal, cabcomp.bonif, cabcomp.impuestos, cabcomp.ivari, cabcomp.ivarni, ' +
