...
TTPeriodo = class(TObject)            // Superclase
  periodo, dfecha, hfecha, pgfecha, phfecha, proteger, activo, estado: string;
  tperiodo: TTable;
 public
  ...
  procedure   Grabar(xperiodo, xdfecha, xhfecha, xpgfecha, xphfecha, xproteger: string);
...
begin
  inherited Create;
  path := '';
  tperiodo := datosdb.openDB('periodo', 'periodo');
end;
...
procedure TTPeriodo.Grabar(xperiodo, xdfecha, xhfecha, xpgfecha, xphfecha, xproteger: string);
...
begin
  if Buscar(xperiodo) then
    ...
    begin
      periodo := ''; dfecha := ''; hfecha := ''; pgfecha := ''; phfecha := ''; proteger := ''; activo := ''; estado := '';
    end;
end;
...
begin
  ...
  p_act := '';
  tperiodo.First;
  while not tperiodo.EOF do
    begin
      if tperiodo.FieldByName('activo').AsString = 'X' then
        begin
...
begin
  ...
  while not tperiodo.EOF do
    begin
      ...
      if (tperiodo.FieldByName('periodo').AsString = xperiodo) and (est = 'S') then
        begin
          tperiodo.Edit;
...
begin
  ...
  while not tperiodo.EOF do
    begin
      if (prot = 'S') and (tperiodo.FieldByName('periodo').AsString = xperiodo) then
        begin
          tperiodo.Edit;
...
begin
  {pardigct.conectar;
  pardigct.getDatos;
  if Length(Trim(pardigct.getPatneto)) > 0 then Result := True else Result := False;
  pardigct.desconectar;}
  Result := True;
end;
...
begin
  Result := datosdb.tranSQL(path, 'SELECT * FROM periodo');
end;

function TTPeriodo.setPeriodosCerrados: TQuery;
// Objetivo...: Determinar características multiempresa
begin
  Result := datosdb.tranSQL(path, 'SELECT * FROM periodo WHERE estado = ' + '''' + 'C' + '''');
end;
...
procedure TTPeriodo.Via(xvia: string);
// Objetivo...: Abrir tperiodos de persistencia para un directorio X
begin
  tperiodo := nil;
  tperiodo := datosdb.openDB('periodo', 'Periodo', '', dbs.dirSistema + xvia);
