...
uses CLibCont, CTablaIva, CCNetos, SysUtils, CListar, DB, DBTables, CBDT, CUtiles, CIDBFM, CProvin, CEmpresas, Classes, CServers2000_Excel;
...
TTIva = class(TTLibrosCont)
  idcompr, tipo, sucursal, numero, cuit, clipro, rsocial, codiva, concepto, ferecep, fecha, codprovin, codmov, tipomov, discriminaIVA, Protegido, Idmov: string;
 ...
 public
  ...
  procedure   Grabar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xclipro, xrsocial, xcodiva, xconcepto, xfecha, xferecep, xcodprovin, xcodmov, xtipomov: string;
                     xnettot, xopexenta, xconnograv, xiva, xivarec, xpercep1, xpercep2, xcdfiscal, xtotoper, xretencion, ximporte: real; xbienuso, xidmov: String); overload;
  ...
  procedure   getDatosEncabezadoInformes; virtual;

  procedure  GuardarNroPagina; overload;
  procedure  GuardarNroPagina(xnumero: Integer); overload;
  function   setNumeroDePagina: Integer;

  procedure   CerrarOperaciones(xcodemp, xdesde, xhasta: String);
  procedure   ReactivarOperaciones(xcodemp, xdesde, xhasta: String);
 private
  ...
  procedure CerrarReactivar(xcodemp, xdesde, xhasta, xmodo: String);
 protected
  ...
  xmes, rsocial1, rsocial2, nrocuit, dir_tel, path, lin, nombre, Modulo, __cont: string;
  ...
  procedure  Exportar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xcodmov: String);
end;
...
procedure TTIva.Grabar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xclipro, xrsocial, xcodiva, xconcepto, xfecha, xferecep, xcodprovin, xcodmov, xtipomov: string;
                     xnettot, xopexenta, xconnograv, xiva, xivarec, xpercep1, xpercep2, xcdfiscal, xtotoper: real);
// Objetivo...: Grabar Atributos del Objeto - Identificar los movimientos automáticos de los ingresados manualmente a través del libro de I.V.A.
var
  f: boolean;
begin
  f := tiva.Filtered;
...
procedure TTIva.Grabar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xclipro, xrsocial, xcodiva, xconcepto, xfecha, xferecep, xcodprovin, xcodmov, xtipomov: string;
                     xnettot, xopexenta, xconnograv, xiva, xivarec, xpercep1, xpercep2, xcdfiscal, xtotoper, xretencion, ximporte: real; xbienuso, xidmov: String);
// Objetivo...: Grabar Atributos del Objeto - 2 Netos
var
  f: boolean;
begin
...
begin
  ...
  tiva.FieldByName('fecha').AsString     := utiles.sExprFecha2000(xfecha);
  tiva.FieldByName('ferecep').AsString   := utiles.sExprFecha2000(xferecep);
...
begin
  ...
  iiva.FieldByName('fecha').AsString    := utiles.sExprFecha2000(xfecha);
  ...
  datosdb.refrescar(iiva);
end;

procedure TTIva.Exportar(xidcompr, xtipo, xsucursal, xnumero, xcuit, xcodmov: String);
// Objetivo...: Exportar Detalle
begin
  if BuscarItems(xidcompr, xtipo, xsucursal, xnumero, xcuit, xcodmov, '001') then Begin
    //utiles.msgError('ss');
    while not iiva.Eof do Begin
      if (iiva.FieldByName('idcompr').AsString <> xidcompr) or (iiva.FieldByName('tipo').AsString <> xtipo) or (iiva.FieldByName('sucursal').AsString <> xsucursal) or (iiva.FieldByName('numero').AsString <> xnumero) or (iiva.FieldByName('cuit').AsString <> xcuit) then Break;
      if datosdb.Buscar(eiva, 'idcompr', 'tipo', 'sucursal', 'numero', 'cuit', 'codmov', 'coditems', iiva.FieldByName('idcompr').AsString, iiva.FieldByName('tipo').AsString, iiva.FieldByName('sucursal').AsString,
                        iiva.FieldByName('numero').AsString, iiva.FieldByName('cuit').AsString, iiva.FieldByName('codmov').AsString, iiva.FieldByName('coditems').AsString) then eiva.Edit else eiva.Append;

      eiva.FieldByName('idcompr').AsString  := iiva.FieldByName('idcompr').AsString;
      eiva.FieldByName('tipo').AsString     := iiva.FieldByName('tipo').AsString;
      eiva.FieldByName('sucursal').AsString := iiva.FieldByName('sucursal').AsString;
      eiva.FieldByName('numero').AsString   := iiva.FieldByName('numero').AsString;
      eiva.FieldByName('cuit').AsString     := iiva.FieldByName('cuit').AsString;
      eiva.FieldByName('codmov').AsString   := iiva.FieldByName('codmov').AsString;
      eiva.FieldByName('coditems').AsString := iiva.FieldByName('coditems').AsString;
      eiva.FieldByName('items').AsString    := iiva.FieldByName('items').AsString;
      eiva.FieldByName('fecha').AsString    := iiva.FieldByName('fecha').AsString;
      eiva.FieldByName('nettot').AsFloat    := iiva.FieldByName('nettot').AsFloat;
      eiva.FieldByName('iva').AsFloat       := iiva.FieldByName('iva').AsFloat;
      eiva.FieldByName('ivarec').AsFloat    := iiva.FieldByName('ivarec').AsFloat;
      try
        eiva.Post;
       except
        eiva.Cancel;
...
begin
  ...
  if datosdb.verificarSiExisteCampo(tiva, 'protegido') then
    protegido := tiva.FieldByName('protegido').AsString;
end;
...
// Objetivo...: Aislar movimientos de I.V.A. (Manuales/Automáticos)
begin
...
// Objetivo...: Calcular I.V.A. a partir del código de movimiento del Neto
begin
...
// Objetivo...: retornar un subset con los movimientos de I.V.A. para un periodo dado
begin
  //utiles.msgerror(tiva.DatabaseName + '   SELECT idcompr, tipo, sucursal, numero, clipro, rsocial, fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper FROM ' + tiva.TableName +  ' WHERE fecha >= ' + '"' + xdf + '"' + ' AND fecha <= ' + '"' + xhf + '"' + ' ORDER BY fecha');
  Result := datosdb.tranSQL(tiva.DatabaseName, 'SELECT idcompr, tipo, sucursal, numero, clipro, rsocial, cuit, fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper, tipomov FROM ' + tiva.TableName +  ' WHERE fecha >= ' + '"' + xdf + '"' + ' AND fecha <= ' + '"' + xhf + '"' + ' ORDER BY fecha');
end;

function TTIva.setIvaFechaRecepcion(xdf, xhf: string): TQuery;
// Objetivo...: retornar un subset con los movimientos de I.V.A. para un periodo dado
begin
  Result := datosdb.tranSQL(tiva.DatabaseName, 'SELECT idcompr, clipro, rsocial, tipo, sucursal, numero, clipro, rsocial, cuit, fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper, tipomov FROM ' + tiva.TableName +  ' WHERE ferecep >= ' + '"' + xdf + '"' + ' AND ferecep <= ' + '"' + xhf + '"' + ' ORDER BY fecha');
end;

function TTIva.setMovimientosIvaFecha(xfecha: string): TQuery;
// Objetivo...: retornar un subset con los movimientos de I.V.A. para un periodo dado
begin
  Result := datosdb.tranSQL(tiva.DatabaseName, 'SELECT * FROM ' + tiva.TableName +  ' WHERE fecha = ' + '"' + utiles.sExprFecha2000(xfecha) + '"' + ' ORDER BY fecha');
end;
...
begin
  ...
  datosDB.tranSQL(dbs.DirSistema + '\' + xvia, 'DELETE FROM ' + tiva.TableName + ' WHERE fecha <= ' + '"' + utiles.sExprFecha2000(fecha) + '"');
  datosDB.tranSQL(dbs.DirSistema + '\' + xvia, 'DELETE FROM ' + iiva.TableName + ' WHERE fecha <= ' + '"' + utiles.sExprFecha2000(fecha) + '"');
  datosdb.closedb(tiva); tiva.Open;
  datosdb.closedb(iiva); iiva.Open;
end;

procedure TTIva.Depurar(xfecha, xtipomov: string);
// Objetivo...: Depurar aquellos movimientos Inferiores al Periodo Seleccionado, seleccionando ademas el tipo de movimiento (automático/manual)
begin
  datosDB.tranSQL(path, 'DELETE FROM ' + tiva.TableName + ' WHERE fecha <= ' + '"' + utiles.sExprFecha2000(xfecha) + '"' + ' AND tipomov = ' + '"' + xtipomov + '"');
  datosdb.refrescar(tiva);
end;
...
// Objetivo...: Determinar la página de inicio
begin
  nropagina := pagini;
end;
...
begin
  ...
  list.Titulo(0, 0, utiles.espacios(394) + 'Hoja Nº: ' + utiles.sLlenarIzquierda((FloatToStr(pag)), 4, '0'), 1, 'Times New Roman, ninguno, 8');
...
begin
  ...
  if op = 'Netos Discriminados en Ventas' then  // Si se trata de un contribuyente distinto efectuamos un salto de página
    begin
      if list.altopag > 0 then
        begin
          list.CompletarPagina;     // Provocamos el Salto de Página
  ...
  list.Linea(0, 0, ' ', 1, 'Arial, normal, 6', salida, 'S');
  list.Linea(0, 0, 'Créd. Resoluciones Varias : ', 1, 'Arial, normal, 10', salida, 'N');
...
begin
  ...
  // 1º Línea de Títulos
  list.Titulo(0, 0, 'Condición Fiscal',1 , 'Arial, cursiva, 9');
  list.Titulo(60, list.lineactual, 'Neto/Tot/Exento',2 , 'Arial, cursiva, 9');
  list.Titulo(78, list.lineactual, 'I.V.A. (Tasa Normal)',3 , 'Arial, cursiva, 9');
  list.Titulo(97, list.lineactual, 'I.V.A. (Tasa Diferencial)',4 , 'Arial, cursiva, 9');
...
// Objetivo...: Listar Linea Informe de I.V.A. discriminado por Condición Fiscal
begin
...
// Objetivo...: Emitir Listado Informe I.V.A. diascriminado por Condición Fiscal
var
  ivaanterior: string;
begin
  // Datos para Iniciar Reporte
  list.IniciarTitulos; Tit_ListCodpfis(salida);
  if op = 'I.V.A. Discriminado en Ventas' then  // Si se trata de un contribuyente distinto efectuamos un salto de página
    begin
      if list.altopag > 0 then
        begin
          list.CompletarPagina;     // Provocamos el Salto de Página
...
begin
  ...
  if (salida = 'P') or (salida = 'I') then Begin
    ...
    if movim = 'Compras' then list.titulo(0, 0, utiles.espacios(40) +'Hoja Nº: ' + utiles.sLlenarIzquierda((FloatToStr(pag)), 4, '0'), 2, 'Times New Roman, ninguno, 8');
    list.Linea(0, 0, ' ',1 , 'Arial, normal, 7', salida, 'S');
  end else Begin
...
begin
  if (salida = 'P') or (salida = 'I') then Begin
    if movim = 'Ventas' then Begin
      ...
      list.linea(112, list.lineactual, 'Percepción' ,8 , 'Arial, cursiva, 8', salida, 'N');
      ...
      list.linea(127, list.lineactual, 'Operación' ,9 , 'Arial, cursiva, 8', salida, 'N');
      list.linea(144, list.lineactual, 'Nº 2784' ,10 , 'Arial, cursiva, 8', salida, 'S');
    end;
    if movim = 'Compras' then Begin
      ...
      list.linea(71, list.lineactual, 'I.V.A.' ,5 , 'Arial, cursiva, 8', salida, 'N');
      list.linea(86, list.lineactual, 'I.V.A.' ,6 , 'Arial, cursiva, 8', salida, 'N');
      list.linea(97, list.lineactual, 'Créd. p/Res.' ,7 , 'Arial, cursiva, 8', salida, 'N');
...
begin
  iniciarTot;
  if fecharecep then r := datosdb.tranSQL(path, 'SELECT fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper FROM ivaventa WHERE ferecep >= ' + '"' + utiles.sExprFecha2000(xdfr) + '"' + ' AND ferecep <= ' + '"' + utiles.sExprFecha2000(xhfr) + '"' + ' ORDER BY ferecep')
     else r := datosdb.tranSQL(path, 'SELECT fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper FROM ivaventa WHERE fecha >= ' + '"' + utiles.sExprFecha2000(xdfr) + '"' + ' AND fecha <= ' + '"' + utiles.sExprFecha2000(xhfr) + '"' + ' ORDER BY fecha');
  r.Open; r.First;
...
begin
  iniciarTot;
  if fecharecep then r := datosdb.tranSQL(path, 'SELECT fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper, retencion, importe FROM ivacompr WHERE ferecep >= ' + '"' + utiles.sExprFecha2000(xdfr) + '"' + ' AND ferecep <= ' + '"' + utiles.sExprFecha2000(xhfr) + '"' + ' ORDER BY ferecep')
     else r := datosdb.tranSQL(path, 'SELECT fecha, ferecep, nettot, opexenta, connograv, iva, ivarec, percep1, percep2, cdfiscal, totoper, retencion, importe FROM ivacompr WHERE fecha >= ' + '"' + utiles.sExprFecha2000(xdfr) + '"' + ' AND fecha <= ' + '"' + utiles.sExprFecha2000(xhfr) + '"' + ' ORDER BY fecha');
  r.Open; r.First;
  if not fecharecep then mesanter := Copy(r.FieldByName('fecha').AsString, 5, 2) else mesanter := Copy(r.FieldByName('ferecep').AsString, 5, 2);
  while not r.EOF do
    begin
...
begin
  ...
  if list.tipolist = 'X' then excel.Visulizar;
  inf_iniciado := False; iva_existe := False;
  if (list.tipolist = 'P') or (list.tipolist = 'I') then list.ImprimirVetical;
end;
...
procedure TTIva.GuardarNroPagina(xnumero: Integer);
// objetivo...: Guardar el ultimo Nro. de Página
Begin
  if modulo = 'C' then AssignFile(archivo, dbs.DirSistema + '\upc.ini') else AssignFile(archivo, dbs.DirSistema + '\upv.ini');
  rewrite(archivo);
...
procedure TTIva.CerrarOperaciones(xcodemp, xdesde, xhasta: String);
// Objetivo...: cerrar operaciones iva
begin
  CerrarReactivar(xcodemp, xdesde, xhasta, 'S');
end;

procedure TTIva.ReactivarOperaciones(xcodemp, xdesde, xhasta: String);
// Objetivo...: reactivar operaciones iva
begin
  CerrarReactivar(xcodemp, xdesde, xhasta, 'A');
end;

procedure TTIva.CerrarReactivar(xcodemp, xdesde, xhasta, xmodo: String);
// Objetivo...: cerrar/reactivar operaciones iva
begin
  empresa.getDatos(xcodemp);
  if not datosdb.verificarSiExisteCampo(tiva.TableName, 'protegido', dbs.DirSistema + '\' + empresa.nomvia) then datosdb.tranSQL(dbs.DirSistema + '\' + empresa.nomvia, 'alter table ' +  tiva.TableName + ' add protegido char(1)');
  datosdb.tranSQL(dbs.DirSistema + '\' + empresa.nomvia, 'update ' + tiva.TableName + ' set protegido = ' + '''' + xmodo + '''' + ' where fecha >= ' + '''' + utiles.sExprFecha2000(xdesde) + '''' + ' and fecha <= ' + '''' + utiles.sExprFecha2000(xhasta) + '''');
end;
