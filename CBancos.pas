...
constructor TTBanco.Create;
begin
  inherited Create;
  tabla    := datosdb.openDB('entbcos', 'codbanco');
end;
...
begin
  ...
  if Length(Trim(tabla.FieldByName('codbanco').AsString)) > 0 then Result := IntToStr(tabla.FieldByName('codbanco').AsInteger + 1) else Result := '1';
  ...
  while not tabla.EOF do
    begin
      ...
      if (ent_excl = 'E') and (orden = 'C') then
        if (tabla.FieldByName('codbanco').AsString >= iniciar) and (tabla.FieldByName('codbanco').AsString <= finalizar) then List.Linea(0, 0, tabla.FieldByName('codbanco').AsString + '     ' + tabla.FieldByName('descrip').AsString, 1, 'Arial, normal, 8', salida, 'S');
      if (ent_excl = 'X') and (orden = 'C') then
        if (tabla.FieldByName('codbanco').AsString < iniciar) or (tabla.FieldByName('codbanco').AsString > finalizar) then List.Linea(0, 0, tabla.FieldByName('codbanco').AsString + '     ' + tabla.FieldByName('descrip').AsString, 1, 'Arial, normal, 8', salida, 'S');
      // Ordenado Alfabéticamente
      if (ent_excl = 'E') and (orden = 'A') then
        if (tabla.FieldByName('descrip').AsString >= iniciar) and (tabla.FieldByName('descrip').AsString <= finalizar) then List.Linea(0, 0, tabla.FieldByName('codbanco').AsString + '     ' + tabla.FieldByName('descrip').AsString, 1, 'Arial, normal, 8', salida, 'S');
      if (ent_excl = 'X') and (orden = 'A') then
        if (tabla.FieldByName('descrip').AsString < iniciar) or (tabla.FieldByName('descrip').AsString > finalizar) then List.Linea(0, 0, tabla.FieldByName('codbanco').AsString + '     ' + tabla.FieldByName('descrip').AsString, 1, 'Arial, normal, 8', salida, 'S');
