...
type
 TTFacturacionCCB = class(TObject)
  ...
  _caran: Real;
  ExcluirLab, SC, DetMontoFijo, exporta_web, listtotalboleta: Boolean;
  listlab: array[1..elementos+5] of String;
  cabfact, detfact, idordenes, parametrosInf, modeloc, cabfactos, liq, datosfact, ctrlImpr, historico, totalesOS, totalesPROF, ordenes_audit, datosImport, cab_auditoria, det_auditoria: TTable;
 ...
 public
  ...
  procedure   Grabar(xperiodo, xidprof, xcodos, xitems, xcodpac, xnombre, xcodanalisis, xorden, xperiodo1, xretiva: string; modifica, guardar_orden_interna: boolean);
 ...
 private
  ...
  ProcesamientoCentral, informe_ivaret, codigo_tomamuestra, datosListadosFact, u_h, nnbu, proceso_central, campo1: Boolean;
  ar1, ar2: TextFile;
  ...
  datosListados, rp, listControl, ExportarDatos,llt, tibase, __historico: Boolean;
  rsql, ressql: TQuery;

  directorio1, diractual1, __host, __us, __pa: String;
...
begin
  ...
  if (interbase = 'S') then begin
    QuitarFiltro;
    if cabfactIB.IndexFieldNames <> 'PERIODO;IDPROF;CODOS' then cabfactIB.IndexFieldNames := 'PERIODO;IDPROF;CODOS';
    if ffirebird.Buscar(cabfactIB, 'PERIODO;IDPROF;CODOS', xperiodo, xidprof, xcodos) then Begin
      getDatos;
      Result  := True;
    end else Begin
      periodo := ''; idprof := ''; codos := '';
...
procedure TTFacturacionCCB.Grabar(xperiodo, xidprof, xcodos, xitems, xcodpac, xnombre, xcodanalisis, xorden, xperiodo1, xretiva: string; modifica, guardar_orden_interna: boolean);
...
begin
  ...
  if (interbase = 'S') then begin
    ...
    if xitems = '001' then Begin
      ...
      if (copy(xorden, 1, 1) = 'R') then begin // Facturación Global, reescribimos - 15/08/2014
        lote.Clear;
        lote.Add('DELETE FROM  ' + __c + '  WHERE periodo = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' AND codos = ' + '"' + xcodos + '"');
        lote.Add('DELETE FROM  ' + __t + '  WHERE periodo = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' AND orden = ' + '"' + xorden + '"' + ' AND codos = ' + '"' + xcodos + '"');
      end;
      ...
      if (copy(xorden, 1, 1) <> 'R') then begin        
        if not (modifica) then ordenint := utiles.sLlenarIzquierda(NuevaOrdenInterna(xperiodo, xidprof, xcodos), 4, '0');
        if (guardar_orden_interna) then GuardarOrdenInterna(xperiodo, xidprof, ordenint);
      end;
    ...
    {utiles.msgError('insert into ' + __t + ' (periodo, idprof, codos, items, orden, codpac, nombre, codanalisis, ref1, retiva) values (' +
      '''' + xperiodo + '''' + ',' +
      '''' + xidprof + '''' + ',' +
      '''' + xcodos + '''' + ',' +
      '''' + xitems + '''' + ',' +
...
begin
  ...
  if (interbase = 'S') then begin
      ...
      if (ordenanter = '') then ordenanter := '1';    
...
begin
  ...
  ffirebird := firebird;
  if (__host = '') then begin
        firebird.getModulo('facturacion');
        __host := firebird.Host;
        __us := firebird.Usuario;
        __pa := firebird.Password;
      end;
  ffirebird.Conectar(__host + 'FACTLABWORK.GDB', __us , __pa);

  {ffirebird.getModulo('facturacion');
  ffirebird.Conectar(ffirebird.Host +  'FACTLABWORK.GDB', ffirebird.Usuario , ffirebird.Password);}
...
begin
  ...
  rsqlIB.Open;
  while not rsqlIB.eof do begin
    lote.Add('insert into detfact_hist (periodo, idprof, codos, items, orden, codpac, nombre, codanalisis, ref1, osiva, retiva, profiva) values (' +
             '''' + xperiodo + '''' + ', ' +
             '''' + rsqlIB.FieldByName('idprof').AsString + '''' + ', ' +
             '''' + rsqlIB.FieldByName('codos').AsString + '''' + ', ' +
...
Begin
  ...
  if (interbase = 'S') then begin
    Result := False;
    ffirebird.Filtrar(detfactIB, 'Periodo = ' + '''' + xperiodo + '''' + ' and Codos = ' + '''' + xcodos + '''');
    if detfactIB.RecordCount > 0 then Result := True;
...
var
  ...
  _totUB, _canUB, unidadNBU, unidad_NBU, __montocache: Real;
  rie: String;
  ...
  if salida <> 'T' then Begin
    ...
    if idanter <> idanter1 then Begin
      if (osretieneiva <> 'S') then begin
        ...
        paciente.getDatos(idprofanter, idanter);
        if Length(Trim(npac)) = 0 then list.LineaTxt(Copy(paciente.nombre, 1, 11) + ' [' + pac_retiva {paciente.Gravadoiva} + ']' + utiles.espacios(15 - Length(Copy(paciente.nombre, 1, 11) + ' [' + pac_retiva {paciente.Gravadoiva} + ']')), False) else list.LineaTxt(Copy(npac, 1, 11) + ' [' + paciente.Gravadoiva + ']' + utiles.espacios(15 - Length(Copy(npac, 1, 11) + ' [' + paciente.Gravadoiva + ']')), False);
      end;
...
begin
  ...
  if ((obsocial.Corteorden = 'S') and (total_orden <> 0) and not (listControl)) or ((listtotalboleta) and (total_orden <> 0)) then begin
    if (salida = 'P') or (salida = 'I') then begin
      list.Linea(0, 0, 'Total Orden: ', 1, 'Arial, negrita, 8', salida, 'N');
    ...
    if (salida = 'T') then begin
      list.LineaTxt(chr(18) + 'Total Orden: ' + utiles.FormatearNumero(floattostr(total_orden)), False);
      list.LineaTxt(' ', True); Inc(lineas); if ControlarSalto then titulo1(periodo, titulo, xcolumnas);
    end;
   ...
   if not rp then Begin
     if salida <> 'T' then Begin
      ...
      if (obsocial.retencioniva > 0) and (profesional.Retieneiva = 'S') then Begin                                   // Obras Sociales con I.V.A.
        ...
        list.Linea(0, 0, ' ', 1, 'Arial, normal, 9', salida, 'N');
        list.Linea(40, list.Lineactual, 'Subtotal Gravado: ', 2, 'Arial, normal, 9', salida, 'N');
        list.Linea(80, list.Lineactual, moneda, 3, 'Arial, normal, 9', salida, 'N');
      ...
      list.LineaTxt('', True); Inc(lineas); if controlarSalto then titulo1(periodo, titulo, columnas);
      if (obsocial.retencioniva > 0) and (profesional.Retieneiva = 'S') then Begin                                   // Obras Sociales con I.V.A.
        list.LineaTxt(CHR18 + utiles.espacios(26) + 'Subtotal Gravado        :             ', False);
        list.LineaTxt(' ' + moneda, False);
...
// Objetivo...: Devolver la Nómina de Obras Sociales Importadas por cada laboratorio
Begin
  if (cabfactIB <> Nil) then Result := ffirebird.getTransacSQL('SELECT codos, idprof FROM cabfact ORDER BY codos') else Result := nil;
end;
  ...
  if (profesional.Buscar(xidprof)) and (interbase = 'S') then Begin
    ...
    while not rsqlIB.Eof do Begin
      ...
      if obsocial.FactNBU = 'S' then Begin
        ...
        lote.Add('delete from detfact where codos = ' + '''' + rsqlIB.FieldByName('codos').AsString + '''' + ' and periodo = ' + '''' + xperiodo + '''' + ' and idprof = ' + '''' + xidprof + '''');
...
begin
  ...
  if (LaboratorioActual <> xlaboratorio) or (Periodo <> xperiodo) then Begin
    ...
    if (length(trim(__host)) > 0) then begin
...
begin
  ...
  if (DirectoryExists(directorio)) or (length(trim(__host)) > 0)  then   // Verificamos que exista el directorio
    if (length(trim(ffirebird.Host)) > 0) or (FileExists(directorio + '\cabfact.db') and FileExists(directorio + '\detfact.db') and FileExists(directorio + '\idordenes.db') and (FileExists(directorio + '\paciente.db'))) {or (FileExists(s + '\FACTLAB.GDB'))} then Begin  // Verificamos que existan los archivos
...
begin
  ...
  //ffirebird.getModulo('facturacion');
  if (__host = '') then begin
    if (ffirebird = nil) then ffirebird := firebird; // ffirebird := TTFirebird.Create;
  ...
  if (length(trim(__host)) = 0) then interbase := 'N' else interbase := 'S';
  ...
  if (interbase = 'S') then begin
    ...
    ffirebird.Desconectar;
    ffirebird.Conectar(__host + 'FACTLABCENT.GDB', __us , __pa);

      utiles.msgError(ffirebird.IBDatabase.DatabaseName);


    //ffirebird.getModulo('facturacion');
    //ffirebird.Conectar(ffirebird.Host + 'FACTLABCENT.GDB', ffirebird.Usuario , ffirebird.Password);
...
begin
  ...
  if (interbase = 'S') then begin
      ...
      while not rsqlIB.EOF do Begin
        ...
        lote.Add('insert into detfact (periodo, idprof, codos, items, orden, codpac, nombre, codanalisis, ref1, retiva, profiva, osiva) values (' +
          '''' + rsqlIB.FieldByName('periodo').AsString + '''' + ',' +
          '''' + rsqlIB.FieldByName('idprof').AsString + '''' + ',' +
          '''' + rsqlIB.FieldByName('codos').AsString + '''' + ',' +
          '''' + rsqlIB.FieldByName('items').AsString + '''' + ',' +
...
// Objetivo...: Devolver las Determinaciones Facturadas en el Periodo para una Determinada Obra Social
Begin
  Result := ffirebird.getTransacSQL('SELECT periodo, codos, codanalisis, idprof, codpac, orden FROM ' + detFactIB.TableName + ' WHERE codos = ' + '"' + xcodos + '"' + ' AND periodo = ' + '''' + xperiodo + '''' + ' ORDER BY codanalisis');
end;
...
Begin
  ...
  if (interbase = 'S') then begin
    ri := ffirebird.getTransacSQL('SELECT DISTINCT IDPROF, NOMBRE FROM ' + detFactIB.TableName + ' WHERE periodo = ' + '"' + xperiodo + '"' + ' AND codos = ' + '"' + xcodos + '"');
    ri.Open; i := 0;
...
// Objetivo...: devolver las ordenes de un determinado periodo y un determinado profesional
Begin
  if ordenes_auditIB <> Nil then Result := ffirebird.getTransacSQL('select * from ordenes_audit where periodo = ' + '"' + xperiodo + '"' + ' and idprof = ' + '"' + xidprof + '"' + ' order by items') else Result := Nil;
end;
