...
begin
  Result := datosdb.tranSQL(tabla.DatabaseName, 'SELECT codigo, items, elemento, resultado, valoresn, imputable, distancia, itemsparalelo, formula FROM plantan WHERE codigo is not null');
end;

function TTPlantillaAnalisisClinicos.setplantanalisis(xcodigo: string): TQuery;
// Objetivo...: Devolver un set de registro con los items
begin
 // utiles.msgError(dir + '  ' + 'SELECT codigo, items, elemento, resultado, valoresn, imputable, distancia, itemsparalelo, formula FROM plantan WHERE codigo = ' + '''' + xcodigo + '''' + ' ORDER BY Items');
  Result := datosdb.tranSQL(tabla.DatabaseName, 'SELECT codigo, items, elemento, resultado, valoresn, imputable, distancia, itemsparalelo, formula FROM plantan WHERE codigo = ' + '''' + xcodigo + '''' + ' ORDER BY Items');
end;
