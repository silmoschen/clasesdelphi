...
uses CEstadisticas, CNomecla, CPaciente, CProfesional, CccteLab, CSolAnalisisFabrissin, Classes,
     SysUtils, DB, DBTables, CVias, CUtiles, CListar, CBDT, CIDBFM, CObrasSociales,
     CDerivacionesAnalisis, CSanatoriosLaboratorios;
...
protected
  ...
  procedure TotalAnalisis(salida: char; msg: string);
end;
...
begin
  ...
  if not InfResumido then Begin
    ...
    List.Titulo(85, list.Lineactual, 'Sanatorio/Clínica', 4, 'Arial, cursiva, 8');
  ...
  while not Q.EOF do
    begin
      ...
      if not InfResumido then Begin
        ...
        sanatorio.getDatos(Q.FieldByName('entidadderiv').AsString);
        ...
        List.Linea(65, list.lineactual, Copy(profesional.nombres, 1, 25), 3, 'Arial, normal, 8', salida, 'N');
        List.Linea(85, list.lineactual, sanatorio.Descrip, 4, 'Arial, normal, 8', salida, 'S');
      end;
      total := total + 1;
    ...
    if not InfResumido then TotalAnalisis(salida, 'Total de Análisis Efectuados:') else TotalAnalisis(salida, descrip);
...
procedure TTInformesEstadisticos.ListPacientesObraSocial(salida: char);
...
procedure TTInformesEstadisticos.TotalAnalisis(salida: char; msg: string);
begin
