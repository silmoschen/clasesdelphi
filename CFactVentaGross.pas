unit CFactVentaGross;

interface

uses CFactVenta, CLPsimpl_Gross, CClienteGross, CArticulosGross, CCobrosTarjetas_Gross, CUtiles, SysUtils, CListar, DBTables, CIDBFM, CAdmNumCompr, CIvaVentaGross;
...
TTFacturaVentaGross = class(TTFacturaVenta)
 ...
 public
  ...
  procedure   Grabar(xidcompr, xtipo, xsucursal, xnumero, xidtitular, xfecha, xremito, xitems, xcodart, xdescrip, xidart, xentregado, xremitoItems: string; xctcc: integer; xsubtotal, xbonificacion, ximpuestos, xivari, xivarni, xsobretasa, xcantidad, xprecio, xdescuento, xdestogeneral, xentrega, xrecargo, xneto: real; xobservacion, xtarjeta, xcliente: string);
...
begin
  ...
  while not r.EOF do Begin
    ...
    if salida = 'T' then Begin
      list.LineaTxt(CHR(15) + utiles.espacios(StrToInt(Copy(list.ExtraerItemsMemoImp(0), 1, 3))), false); list.LineaTxt(utiles.espacios(Margen), False); list.ImporteTxt(r.FieldByName(Trim(Copy(list.ExtraerItemsMemoImp(0), 5, 10))).AsFloat, 7, 2, False);
      list.LineaTxt(utiles.espacios(StrToInt(Copy(list.ExtraerItemsMemoImp(1), 1, 3))), false); list.LineaTxt(r.FieldByName('codart').AsString + '-' + r.FieldByName(Trim(Copy(list.ExtraerItemsMemoImp(1), 5, 10))).AsString + utiles.espacios(43 - Length(Trim(r.FieldByName('codart').AsString + '-' + r.FieldByName(Trim(Copy(list.ExtraerItemsMemoImp(1), 5, 10))).AsString))) + utiles.espacios(EspaciosDescripcion),  False);
    ...
    Inc(itemsimpresos);
    totalFactura := totalFactura + ((r.FieldByName('cantidad').AsFloat * r.FieldByName('precio').AsFloat) - r.FieldByName('descuento').AsFloat);
    r.Next;
  end;
  ...
  For i := 1 to (cantidaditems - itemsimpresos) do Begin
    ...
    if salida = 'T' then list.LineaTxt(CHR(18) + '', true);
  end;
end;
...
Begin
  ...
  cobrostarj.Registrar(xidcompr + xtipo + xsucursal + xnumero, Copy(xfecha, 4, 2) + '/' + Copy(utiles.sExprFecha2000(xfecha), 1, 4), xfecha, xtarjeta, xidtitular, 'A', xcliente, xsubtotal);
