...
TTitular = class(TTPersona)
  telefono, email, contrato, M, F, P, Fallecido: string;
  tit: TTable;
 public
  ...
  procedure   Grabar(xidtitular, xnombre, xdomicilio, xcontrato, xtelefono, xemail, xM, xF, xP, xfallecido: string);
...
begin
  ...
  tperso   := datosdb.openDB('titular', 'Idtitular');
  tit     := datosdb.openDB('titularh', 'Idtitular');
  if not datosdb.verificarSiExisteCampo('titularh', 'fallecido', dbs.DirSistema + '\arch') then
    datosdb.tranSQL(dbs.DirSistema + '\arch', 'alter table titularh add fallecido char(50)');
end;
...
procedure TTitular.Grabar(xidtitular, xnombre, xdomicilio, xcontrato, xtelefono, xemail, xM, xF, xP, xfallecido: string);
...
begin
  if Buscar(xidtitular) then
    ...
    begin
      contrato := ''; M := ''; F := ''; P := ''; telefono := ''; email := ''; fallecido := '';
    end;
...
begin
  tperso.IndexName := 'Titular_Titular';
  tperso.FindNearest([xexpr]);
end;
...
begin
  ...
  tperso.FieldByName('idtitular').DisplayLabel := 'Cód.'; tperso.FieldByName('nombre').DisplayLabel := 'Nombre del Titular'; tperso.FieldByName('direccion').DisplayLabel := 'Dirección'; 
