...
TTDericacionAnalisis = class(TObject)            // Superclase
...
function derivanalisis: TTDericacionAnalisis;

implementation

var
  xderivanalisis: TTDericacionAnalisis = nil;

constructor TTDericacionAnalisis.Create;
begin
  inherited create;
  if dbs.BaseClientServ = 'N' then tabla := datosdb.openDB('entidades', 'Identidad');
  if dbs.BaseClientServ = 'S' then tabla := datosdb.openDB('entidades', 'Identidad', '', dbs.baseDat_N);
end;

destructor TTDericacionAnalisis.Destroy;
begin
  inherited Destroy;
end;

procedure TTDericacionAnalisis.Grabar(xIdentidad, xdescrip: string);
...
procedure TTDericacionAnalisis.Borrar(xIdentidad: string);
...
function TTDericacionAnalisis.Buscar(xIdentidad: string): boolean;
...
procedure  TTDericacionAnalisis.getDatos(xIdentidad: string);
...
function TTDericacionAnalisis.Nuevo: string;
...
procedure TTDericacionAnalisis.BuscarPorCodigo(xexp: string);
begin
  tabla.IndexFieldNames := 'Identidad';
  tabla.FindNearest([xexp]);
end;

procedure TTDericacionAnalisis.BuscarPorNombre(xexp: string);
begin
  tabla.IndexFieldNames := 'Descrip';
  tabla.FindNearest([xexp]);
end;

procedure TTDericacionAnalisis.Enccol;
// Objetivo...: setear nombres de columnas
begin
  tabla.FieldByName('Identidad').DisplayLabel := 'Cód.'; tabla.FieldByName('descrip').DisplayLabel := 'Descripción';
end;

procedure TTDericacionAnalisis.conectar;
...
procedure TTDericacionAnalisis.desconectar;
...
function derivanalisis: TTDericacionAnalisis;
begin
  if xderivanalisis = nil then
    xderivanalisis := TTDericacionAnalisis.Create;
  Result := xderivanalisis;
end;
