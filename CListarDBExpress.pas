...
TTListar = class(TObject)            // Superclase
  altopag, m, pagina, reserva, esp, altotit, LineasSubtotales, largoImpresionMemo: integer; Comprimida, visruptura: Boolean;
 ...
 private
  ...
  no_saltar_pagina, saltar_pagina, salto_de_pagina, memo_iniciado, list_seteado, saltar_sinnumerar: boolean;
  archivo: TextFile;
...
begin
  Assign(archivo, dbs.DirSistema + '\list.txt');
  Rewrite(archivo);
  items := 0; sitems := 0;
end;
...
begin
  ...
  if salto then WriteLn(archivo, utiles.StrQuitarCaracteresEspeciales(linea){+ 'S'}) else Write(archivo, utiles.StrQuitarCaracteresEspeciales(linea));
