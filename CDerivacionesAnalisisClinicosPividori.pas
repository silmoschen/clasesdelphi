unit CDerivacionesAnalisisClinicosPividori;

interface

uses CBDT, SysUtils, DBTables, CUtiles, CListar, CIDBFM, Classes, CTitulos,
     CDerivacionesAnalisis, CPaciente, CNomecla, CObrasSociales, CCBloqueosLaboratorios;

type

TTDerivAnalisis = class
  Nroderiv, Fecha, Identidad, Encabezado, Pie: String;
  Existe: Boolean;
  cabderiv, detderiv, formato: TTable;
 public
  ...
  function    setDerivaciones(xnroderiv: String): TStringList;
  function    setDerivEntidad(xidentidad: String): TStringList;
...
begin
  ...
  if dbs.BaseClientServ = 'S' then Begin
    cabderiv := datosdb.openDB('cabderivacion', '', '', dbs.baseDat);
    detderiv := datosdb.openDB('detderivacion', '', '', dbs.baseDat);
    formato  := datosdb.openDB('formatoderivacion', '', '', dbs.baseDat);
  end;
end;
...
Begin
  ...
  if xitems = utiles.sLlenarIzquierda(IntToStr(xcantitems), 2, '0') then datosdb.tranSQL(detderiv.DatabaseName, 'delete from ' + detderiv.TableName + ' where nroderiv = ' + '''' + xnroderiv + '''' + ' and items > ' + '''' + xitems + '''');
end;
...
Begin
  if cabderiv.RecordCount = 0 then Result := '1' else Begin
    cabderiv.Last;
    Result := IntToStr(cabderiv.FieldByName('nroderiv').AsInteger + 1);
  end;
end;

function TTDerivAnalisis.setDerivaciones(xnroderiv: String): TStringList;
...
Begin
  ...
  if BuscarItems(xnroderiv, '01') then Begin
    while not detderiv.Eof do Begin
      ...
      l.Add(detderiv.FieldByName('items').AsString + detderiv.FieldByName('protocolo').AsString + detderiv.FieldByName('codigo').AsString + detderiv.FieldByName('codos').AsString + detderiv.FieldByName('codpac').AsString + detderiv.FieldByName('itemsprot').AsString);
      detderiv.Next;
    end;
  end;
  Result := l;
...
function  TTDerivAnalisis.setDerivEntidad(xidentidad: String): TStringList;
...
Begin
  ...
  if cabderiv.FindKey([xidentidad]) then Begin
    while not cabderiv.Eof do Begin
      ...
      l.Add(cabderiv.FieldByName('nroderiv').AsString + utiles.sFormatoFecha(cabderiv.FieldByName('fecha').AsString) + cabderiv.FieldByName('identidad').AsString);
      cabderiv.Next;
    end;
  end;
...
Begin
  ...
  getDatos(xnroderiv);
  derivanalisis.getDatos(identidad);
  ...
  list.RemplazarEtiquetasEnMemo('#entidad', derivanalisis.Descrip);
...
Begin
  ...
  while not cabderiv.Eof do Begin
    ...
    list.Linea(10, list.Lineactual, utiles.sFormatoFecha(cabderiv.FieldByName('fecha').AsString), 2, 'Arial, negrita, 9', salida, 'N');
...
Begin
  if BuscarItems(xnroderiv, '01') then Begin
    while not detderiv.eof do Begin
      ...
      list.Linea(73, list.Lineactual, obsocial.nombre, 5, 'Arial, normal, 8', salida, 'S');
