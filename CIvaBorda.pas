...
uses CLibCont, CTablaIva, CCNetos, SysUtils, CListar, DB, DBTables, CBDT, CUtiles, CIDBFM, CProvin, CEmpresas, Classes, CComprob;
...
TTIva = class(TTLibrosCont)
 ...
 public
  ...
  procedure   ListComprobante(op: string; salida: char);
 ...
 protected
  ...
  procedure  Tit_Comprobantes(tipolist: char);
end;
...
begin
  ...
  finales[1] := finales[1] + totNettot;
  finales[2] := finales[2] + totIva;
  finales[3] := finales[3] + totIvarec;
...
begin
  ...
  if op = 'Netos Discriminados en Ventas' then Begin // Si se trata de un contribuyente distinto efectuamos un salto de página
    if list.altopag > 0 then begin
  ...
  while not TSQL.EOF do begin
    ...
    netoanterior := TSQL.FieldByName('codmov').AsString;
    if TSQL.FieldByName('tipomov').AsString <> 'X' then Begin
  ...
  list.Linea(56, list.Lineactual, '---------------------------------------------------------------------------', 2, 'Arial, normal, 11', salida, 'S');
  list.Linea(0, 0, 'Subtotal: ', 1, 'Arial, normal, 11', salida, 'N');
  list.importe(70, list.lineactual, '', finales[1], 2, 'Arial, normal, 11');
  list.importe(90, list.lineactual, '', finales[2], 3, 'Arial, normal, 11');
  list.importe(110, list.lineactual, '', finales[3], 4, 'Arial, normal, 11');
  list.Linea(111, list.Lineactual, '', 5, 'Arial, normal, 11', salida, 'S');
  finales[1] := 0; finales[2] := 0; finales[3] := 0;
...
begin
  ...
  list.Titulo(65, list.lineactual, 'Neto1',2 , 'Arial, cursiva, 9');
  list.Titulo(84, list.lineactual, 'Neto2',3 , 'Arial, cursiva, 9');
  list.Titulo(105, list.lineactual, 'Total',4 , 'Arial, cursiva, 9');

  list.Titulo(0, 0, list.linealargopagina(tipolist), 1, 'Arial, normal, 11');
end;

procedure TTIva.Tit_Comprobantes(tipolist: char);
{Objetivo....: Emitir los Títulos del Listado}
begin
  pag := pag + 1;
  ListDatosEmpresa(tipolist);
  list.Titulo(0, 0, 'Totales I.V.A. Disc. por Comprobantes', 1, 'Arial, negrita, 14');
  list.Titulo(0, 0, utiles.espacios(394) + 'Hoja Nº: ' + utiles.sLlenarIzquierda((FloatToStr(pag)), 4, '0'), 1, 'Times New Roman, ninguno, 8');
  list.Titulo(0, 0, ' ',1 , 'Arial, normal, 7');
  // 1º Línea de Títulos
...
begin
  ...
  list.importe(71, list.lineactual, '', totNettot, 3, 'Arial, normal, 10');
  list.importe(90, list.lineactual, '', totNeto2, 4, 'Arial, normal, 10');
  list.importe(110, list.lineactual, '', totOper, 5, 'Arial, normal, 10');
  list.Linea(111, list.lineactual, ' ', 6, 'Arial, normal, 10', salida, 'S');
  finales[1] := finales[1] + totNettot;
  finales[2] := finales[2] + totNeto2;
  finales[3] := finales[3] + totOper;
  totNettot := 0; totNeto2 := 0; totOper := 0;
end;
...
begin
  ...
  list.IniciarTitulos;
  espacios := 2;
  Tit_ListCodpfis(salida);
  if op = 'I.V.A. Discriminado en Ventas' then Begin // Si se trata de un contribuyente distinto efectuamos un salto de página
    if list.altopag > 0 then begin
      list.CompletarPagina;     // Provocamos el Salto de Página
      list.Linea(0, 0, list.linealargopagina(salida), 1, 'Arial, normal, 11', salida, 'N');
      list.IniciarNuevaPagina;
  ...
  while not TSQL.EOF do begin
    ...
    ivaanterior := TSQL.FieldByName('codiva').AsString;
    netos.getDatos(tsql.FieldByName('codmov').AsString);
    tabliva.getDatos(netos.codiva);
    if (Copy(tsql.FieldByName('sucursal').AsString, 1, 1) <> '-') and (tabliva.Ncol = '1') then totNettot := totNettot + TSQL.FieldByName('nettot').AsFloat else
      totNeto2 := totNeto2 + TSQL.FieldByName('nettot').AsFloat;
    totOper := totOper + TSQL.FieldByName('totoper').AsFloat;
    TSQL.Next;
  end;
  if totNettot <> 0 then ListarIva(ivaanterior, salida);

  list.Linea(0, 0, '', 1, 'Arial, normal, 8', salida, 'S');
  list.Linea(0, 0, 'Subtotal:', 1, 'Arial, normal, 10', salida, 'N');
  list.importe(71, list.lineactual, '', finales[1], 2, 'Arial, normal, 10');
  list.importe(90, list.lineactual, '', finales[2], 3, 'Arial, normal, 10');
  list.importe(110, list.lineactual, '', finales[3], 4, 'Arial, normal, 10');
  list.Linea(111, list.lineactual, ' ', 5, 'Arial, normal, 10', salida, 'S');
...
procedure TTIva.ListComprobante(op: string; salida: char);
// Objetivo...: Emitir Listado Informe I.V.A. diascriminado por Condición Fiscal
var
  ivaanterior: string;
begin
  ...
  list.IniciarTitulos;
  espacios := 2;
  Tit_Comprobantes(salida);
  if Pos(op, 'Ventas') > 0 then Begin // Si se trata de un contribuyente distinto efectuamos un salto de página
    if list.altopag > 0 then begin
      list.CompletarPagina;     // Provocamos el Salto de Página
      list.Linea(0, 0, list.linealargopagina(salida), 1, 'Arial, normal, 11', salida, 'N');
      list.IniciarNuevaPagina;
  ...
  TSQL.Open; TSQL.First;
  ivaanterior := TSQL.FieldByName('idcompr').AsString;
  comprobante.conectar;
  while not TSQL.EOF do begin
    if TSQL.FieldByName('idcompr').AsString <> ivaanterior then Begin
      comprobante.getDatos(ivaanterior);
      ListarIva(ivaanterior + ' - ' + comprobante.Descrip, salida);
    end;
    ivaanterior := TSQL.FieldByName('idcompr').AsString;
    netos.getDatos(tsql.FieldByName('codmov').AsString);
    tabliva.getDatos(netos.codiva);
