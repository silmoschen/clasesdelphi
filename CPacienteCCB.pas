...
var
  rsql: TQuery;
begin
  rsql := datosdb.tranSQL('select max(cast(trim(codpac) as float)) as id from paciente where idprof = ' + '''' + xidprof + '''');
  rsql.Open;
  ...
  if tpaciente.RecordCount = 0 then Result := '1' else Begin
    tpaciente.Last;
    Result := IntToStr(StrToInt(trim(tpaciente.FieldByName('codpac').AsString)) + 1);
  end;
  }
end;
