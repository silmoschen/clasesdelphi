...
begin
  ...
  tabla2.FieldByName('codpac').AsString   := trim(xcodigo);
  ...
  inherited Grabar(trim(xcodigo), xnombre, xdomicilio, '', '');
  tperso.Edit;
...
begin
  result := datosdb.tranSQL('select * from pacienteint');
end;
...
begin
   ...
   rssql.Open;
   if (rssql.RecordCount = 0) then result := '1' else result := trim(floattostr(rssql.fields[0].AsFloat + 1));
