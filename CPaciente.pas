...
TTPaciente = class(TTPersona)
 ...
 public
  ...
  function    verificarPacientePorDocumento(xnrodoc: string): boolean;
...
procedure TTPaciente.AjustarNroAfiliado(xcodigo, xnroafiliado: string);
// Objetivo...: Ajustamos Nro Afiliado
begin
...
function TTPaciente.getPacientes: TQuery;
// Objetivo...: cerrar tablas de persistencia
begin
  Result := datosdb.tranSQL('select c.codpac, c.nombre, d.nrodoc, d.email from paciente c, pacienth d where c.codpac = d.codpac');
end;
...
function TTPaciente.verificarPacientePorDocumento(xnrodoc: string): boolean;
var
  r: boolean;
begin
  tabla2.IndexFieldNames := 'NRODOC';
  if (datosdb.Buscar(tabla2, 'NRODOC', xnrodoc)) then begin
    getDatos(tabla2.FieldByName('codpac').asstring);
    r := true;
  end else begin
    r := false;
  end;
  tabla2.IndexFieldNames := 'CODPAC';
  result := r;
end;
