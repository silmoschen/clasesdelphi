...
var
  ...
  var1, var2, var3, var4, var5, expediente, nroaut, iretiva: string;
  ...
  if (archivos) then begin
    ...
    for m := 1 to lista_cab.Count do begin
      ...
      if (length(trim(linea1)) > 0) and (length(trim(copy(linea1, 1, 29))) > 10) then begin
        ...
        if cabref.FindKey([iidonline]) then begin
          datosdb.tranSQL(cabref.DatabaseName, 'delete from cab_ref where nroauditoria = ' + '''' + nuevo_nroauditoria + '''');
          cabref.Refresh;
        end;
        ...
        if (lista_pac.Count > 0) then begin
          for k := 1 to lista_pac.Count do begin
            if (trim(copy(lista_pac.Strings[k-1], 9, 15)) = trim(inrodoc)) then begin
              ...
              iretiva := copy(lista_pac.Strings[k-1], 148, 1);
              break;
            end;
          end;

          // Damos de alta paciente
          padron.Guardar(icodos, inrodoc, inombre, iretiva);
        end;

        // Damos de Alta la Orden
        auditoriacb.Registrar(nuevo_nroauditoria, utiles.sFormatoFecha(ifecha), icodos, iidzona, inrodoc, inombre, iidprof, '', '', '', '', icodos, utiles.sFormatoFecha(ifepedido), iprofcab, iiddiag, iobservacion, false, 'S', ianulada, expediente, nroaut);
