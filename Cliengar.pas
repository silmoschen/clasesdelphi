...
TTCliengar = class(TTCliente)          // Clase TVendedor Heredada de Persona
 ...
 public
  ...
  constructor Create(xcodigo, xnombre, xdomicilio, xcp, xorden, xatresp, xdomcob, xtelcom, xtelcob, xtelalt, xcodpfis, xnrocuit, xemail, xnrodoc, xdocgarante: string; xingreso: real);
  ...
  procedure   GuardarObservacion(xobservacion: String);
...
constructor TTCliengar.Create(xcodigo, xnombre, xdomicilio, xcp, xorden, xatresp, xdomcob, xtelcom, xtelcob, xtelalt, xcodpfis, xnrocuit, xemail, xnrodoc, xdocgarante: string; xingreso: real);
...
procedure TTCliengar.GuardarObservacion(xobservacion: String);
// Objetivo...: Guardar Observacion
Begin
  if tabla3.FindKey([codigo]) then tabla3.Edit else tabla3.Append;
  tabla3.FieldByName('observacion').AsString := xobservacion;
  try
    tabla3.Post
   except
    tabla3.Cancel
...
function TTCliengar.getObservacion: String;
// Objetivo...: recuperar observacion
Begin
  if tabla3.FindKey([codigo]) then Result := tabla3.FieldByName('observacion').AsString else Result := '';
end;
...
begin
  inherited conectar;
  tperso.FieldByName('tarjeta').DisplayLabel := 'Tarjeta Marquez Card'; tperso.FieldByName('domicilio').DisplayLabel := 'Dirección';
...
begin
  if xclientegar = nil then
    xclientegar := TTCliengar.Create('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 0);
  Result := xclientegar;
end;
