...
begin
  if dbs.StoredProc = 'S' then Begin
    stp := datosdb.crearStoredProc(dbs.baseDat, 'buscar_nroimportante');
    stp.ParamByName('nrotel').AsString := xnrotel;
    stp.ExecProc;
    if stp.ParamByName('resultado').AsInteger = 0 then result := false else result := true;
    stp.Free;
...
begin
  if dbs.StoredProc = 'S' then Begin
    if (BuscarNroImportante(xnrotel)) then marca := '' else marca := 'I';
    stp := datosdb.crearStoredProc(dbs.baseDat, 'registrar_nroraro');
    stp.ParamByName('nrotel').AsString  := xnrotel;
    stp.ParamByName('nroraro').AsString := marca;
    stp.ExecProc;
...
// objetivo...: registrar observación
Begin
  //if dbs.StoredProc = 'S' then Begin
...
begin
  if conexiones = 0 then Begin
    ...
    tperso.FieldByName('nrotel').DisplayLabel := 'Nº Teléfono'; tperso.FieldByName('fecha').DisplayLabel := 'Fecha'; tperso.FieldByName('barrio').DisplayLabel := 'Barrio'; tperso.FieldByName('email').DisplayLabel := 'Email'; tperso.FieldByName('ciudad').DisplayLabel := 'Ciudad'; tperso.FieldByName('localidad').DisplayLabel := 'Localidad';
    tperso.FieldByName('Nrocliente').DisplayLabel := 'Número'; tperso.FieldByName('Telalt').DisplayLabel := 'Tél. Alternativo'; tperso.FieldByName('Nombre').DisplayLabel := 'Nombre'; tperso.FieldByName('direccion').DisplayLabel := 'Dirección'; tperso.FieldByName('fechanac').DisplayLabel := 'Fecha Nac.'; tperso.FieldByName('codcli').DisplayLabel := 'Código';
