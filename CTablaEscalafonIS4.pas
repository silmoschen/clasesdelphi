...
begin
  tabla1 := datosdb.openDB('escalafon', '');
  tabla2 := datosdb.openDB('itemsescalafon', '');
  tabla3 := datosdb.openDB('puntajeescala', '');
end;
...
begin
  if BuscarItems(xitems) then Begin
    tabla1.Delete;
    datosdb.closeDB(tabla1); tabla1.Open;
    datosdb.tranSQL('delete from ' + tabla2.TableName + ' where items = ' + '''' + xitems + '''');
...
begin
  ...
  Result := datosdb.Buscar(tabla2, 'Items', 'Subitems', xitems, xsubitems);
end;
...
begin
  ...
  if Length(Trim(xitems)) > 0 then Begin
    datosdb.Filtrar(tabla2, 'items = ' + '''' + xitems + '''');
    tabla2.First;
    ...
    while not tabla1.Eof do Begin
      datosdb.Filtrar(tabla2, 'items = ' + '''' + tabla1.FieldByName('items').AsString + '''');
      tabla2.First;
      while not tabla2.Eof do Begin
        objeto               := TTEscalafon.Create;
        objeto.Items         := tabla2.FieldByName('items').AsString;
...
var
  ...
  var
    l: Boolean;
  Begin
    datosdb.Filtrar(tabla3, 'items = ' + '''' + tabla2.FieldByName('items').AsString + '''' + ' and subitems = ' + '''' + tabla2.FieldByName('subitems').AsString + '''');
...
begin
  ...
  if Length(Trim(xitems)) > 0 then datosdb.Filtrar(tabla1, 'items = ' + '''' + xitems + '''');
  tabla1.First;
  while not tabla1.Eof do Begin
...
begin
  Result := datosdb.Buscar(tabla3, 'items', 'subitems', 'linea', xitems, xsubitems, xlinea);
end;
...
begin
  ...
  if xlinea = utiles.sLlenarIzquierda(IntToStr(xcantitems), 2, '0') then Begin
    datosdb.tranSQL('delete from ' + tabla3.TableName + ' where items = ' + '''' + xitems + '''' + ' and  subitems = ' + '''' + xsubitems + '''' + ' and linea > ' + '''' + xlinea + '''');
...
begin
  datosdb.tranSQL('delete from ' + tabla3.TableName + ' where items = ' + '''' + xitems + '''' + ' and  subitems = ' + '''' + xsubitems + '''');
  datosdb.closeDB(tabla3); tabla3.Open;
end;
...
begin
  l := TObjectList.Create;
  datosdb.Filtrar(tabla3, 'items = ' + '''' + xitems + '''' + ' and subitems = ' + '''' + xsubitems + '''');
  tabla3.First;
  while not tabla3.Eof do Begin
    objeto                  := TTEscalafon.Create;
...
Begin
  ...
  datosdb.Filtrar(tabla1, 'requerido = ' + '''' + 'S' + '''');
  tabla1.First;
  while not tabla1.Eof do Begin
