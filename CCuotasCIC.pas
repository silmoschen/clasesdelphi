...
Begin
  ...
  for i := 1 to 1 { difanios + 1} do Begin
    ...
    if salida <> 'T' then Begin
      list.Linea(0, 0, 'Período: ' + IntToStr(anioini) + '-' + IntToStr(aniofinal), 1, 'Arial, normal, 10', salida, 'S');
      list.Linea(0, 0, '', 1, 'Arial, negrita, 5', salida, 'S');
    end else Begin
      list.LineaTxt('Período: ' + IntToStr(anioini) + '-' + IntToStr(aniofinal), True);
      list.LineaTxt('', True);
    end;
...
Begin
  ...
  if (salida = 'X') then begin
    ...
    Inc(c1); l1 := Trim(IntToStr(c1));
    excel.setString('a' + l1, 'a' + l1, 'Razón Social', 'Arial, normal, 8');
  ...
  for k := 1 to difanios + 1 do Begin
    ...
    if (salida = 'X') then begin
      Inc(c1); l1 := Trim(IntToStr(c1));
      excel.setString('a' + l1, 'a' + l1, 'Año:  ' + IntToStr(anioini), 'Arial, negrita, 9');
    end;
...
Begin
  ...
  if salida <> 'T' then Begin
    ...
    List.Titulo(0, 0, ' ', 1, 'Arial, negrita, 5');
    List.Titulo(0, 0, 'Razón Social - Plan', 1, 'Arial, cursiva, 8');
    ...
    List.LineaTxt(' ', True);
    List.LineaTxt('Razón Social - Plan            Cuotas Atrazadas                Cant.     Monto', True);
    List.LineaTxt('------------------------------------------------------------------------------', True);
    List.LineaTxt(' ', True);
  end;
