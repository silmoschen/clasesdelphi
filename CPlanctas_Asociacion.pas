unit CPlanctas_Asociacion;

interface

uses CRegCont, CPlanctas, CLDiario, capitul, CUtiles, SysUtils, DB, DBTables, CBDT, Forms, CIDBFM;

type

TTGestCuentas = class(TTRegCont)
 public
  ...
  function    CopiarMovimientos(codctasum: string; xnivel: byte): boolean;
...
begin
  ...
  utiles.DarDeAlta(fmCuentas.Codcta.Text);
  fmCuentas.Nivel             := xNivel;
  {Transfiero al Formulario para dar de Alta; confirmo Alta}
  fmCuentas.Imputable         := planctas.Imputable;
  if CodigoSumarizaOk(fmCuentas.Sumatoria.Caption, xNivel) then
    begin
      if utiles.DarDeAlta('Cód. Cta. ' + fmCuentas.Codcta.Text) then
        begin
          ...
          if (xNivel <> 5) or not CopiarMovimientos(fmCuentas.Sumatoria.Caption, xNivel) then fmCuentas.ShowModal else
             begin
               fmCuentas.ShowModal;
...
begin
  ...
  if xNivel > 1 then
    begin
      ...
      if not planctas.Buscar(sumatoria) then
        begin
...
function TTGestCuentas.CopiarMovimientos(codctasum: string; xnivel: byte): boolean;
{Objetivo...: Chequear si hay que cambiar movimiento, en caso verdadero, proceder}
begin
  Result := False;
  if ldiario.verifCuenta(codctasum) then
    if utiles.msgSiNo('Copiar Movimientos a la Cuenta ?') then Result := True;
end;
...
var
  sumariza, ctsum, error: string;
  Borrar, Eliminar: Boolean;
  Nivel : Byte; reg: integer;
begin
  Borrar := True; Eliminar := True;
  reg    := planctas.planctas.RecNo - 2;
  ...
  if planctas.Buscar(cta1) then
    begin
      ...
      if planctas.BuscarSumariza(sumariza) then
        if (planctas.planctas.FieldByName('nivel').AsInteger = 1) and (planctas.planctas.RecordCount > 1) then
          begin
            planctas.planctas.Next;
            if (planctas.planctas.FieldByName('sumariza').AsString = sumariza) and not (planctas.planctas.EOF) then
              begin
                Borrar := False;
        ...
        if planctas.Buscar(cta1) then
          if planctas.planctas.FieldByName('nivel').AsInteger > 1 then
            begin
              error  :=  'La Cuenta Seleccionada tiene Ctas. Subordinadas ...!';
              Borrar := False;
            end
      ...
      if not (Borrar) or not (Eliminar) then
        begin
        ...
        begin
          if utiles.BajaRegistro('Cód. Cta. ' + cta1) then
            begin
      ...
      planctas.planctas.First;
      if not (Borrar) or not (Eliminar) then planctas.Buscar(cta1) else planctas.planctas.MoveBy(reg);
      planctas.planctas.Refresh;
    end;
end;
