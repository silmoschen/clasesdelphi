...
uses CIva, CProve, CEmpresas, CTablaIva, SysUtils, DB, DBTables, CBDT, CUtiles,
     ...
     CCombustibles_Espiga;

type

TTIvacompra = class(TTIva)
  retencion, importe, Litros, Itc, Tvg, Iva3337: Real;
  idcomb: String;
 public
  ...
  procedure   RegistrarCombustible(xidcompr, xtipo, xsucursal, xnumero, xcuit, xidcom: string; xlitros, xitc, xtvg, xiva3337: Real);
  procedure   getDatosCombustible(xidcompr, xtipo, xsucursal, xnumero, xcuit: string);
...
begin
  ...
  if (salida = 'I') or (salida = 'P') then Begin
    ...
    list.Titulo(0, 0, ' ', 1, 'Arial, normal, 8'); list.Titulo(espacios, list.Lineactual, 'Fecha',2 , 'Arial, cursiva, 8');
    list.Titulo(17, list.lineactual, 'Comprobante' + utiles.espacios(18) + 'Proveedor',3 , 'Arial, cursiva, 8');
    list.Titulo(70, list.lineactual, 'Litros', 4, 'Arial, cursiva, 8');
    list.Titulo(81, list.lineactual, 'Neto', 5, 'Arial, cursiva, 8');
    list.Titulo(91, list.lineactual, 'I.V.A.', 6, 'Arial, cursiva, 8');
    list.Titulo(100, list.lineactual, 'Total 1', 7, 'Arial, cursiva, 8');
    list.Titulo(111, list.lineactual, 'I.T.C.', 8, 'Arial, cursiva, 8');
    list.Titulo(120, list.lineactual, 'T.V.G.', 9, 'Arial, cursiva, 8');
    list.Titulo(130, list.lineactual, 'Total 2', 10, 'Arial, cursiva, 8');
    list.Titulo(139, list.lineactual, 'IVA 3337', 11, 'Arial, cursiva, 8');
    list.Titulo(150, list.lineactual, 'Total 3', 12, 'Arial, cursiva, 8');
  ...
  if salida = 'X' then Begin
    ...
    excel.setString('b' + ff, 'b' + ff, 'Comprobante', 'Arial, normal, 8');
    excel.setString('c' + ff, 'c' + ff, 'Proveedor', 'Arial, normal, 8');
    excel.Alinear('d' + ff, 'f' + ff, 'D');
    excel.setString('d' + ff, 'd' + ff, 'Litros', 'Arial, normal, 8');
    excel.Alinear('e' + ff, 'e' + ff, 'D');
    excel.setString('e' + ff, 'e' + ff, 'Neto', 'Arial, normal, 8');
    excel.Alinear('f' + ff, 'f' + ff, 'D');
    excel.setString('f' + ff, 'f' + ff, 'I.V.A.', 'Arial, normal, 8');
    excel.Alinear('g' + ff, 'g' + ff, 'D');
    excel.setString('g' + ff, 'g' + ff, 'Total 1', 'Arial, normal, 8');
    excel.Alinear('h' + ff, 'h' + ff, 'D');
    excel.setString('h' + ff, 'h' + ff, 'I.T.C.', 'Arial, normal, 8');
    excel.Alinear('i' + ff, 'i' + ff, 'D');
    excel.setString('i' + ff, 'i' + ff, 'T.V.G.', 'Arial, normal, 8');
    excel.Alinear('j' + ff, 'j' + ff, 'D');
    excel.setString('j' + ff, 'j' + ff, 'Total 2', 'Arial, normal, 8');
    excel.Alinear('k' + ff, 'k' + ff, 'D');
    excel.setString('k' + ff, 'k' + ff, 'IVA 3337', 'Arial, normal, 8');
    excel.Alinear('l' + ff, 'l' + ff, 'D');
    excel.setString('l' + ff, 'l' + ff, 'Total 3', 'Arial, normal, 8');
...
begin
  ...
  if (salida = 'P') or (salida = 'I') then Begin
   if Trim(leyenda) = 'Subtotales:' then
    ...
    list.PrintLn(0, 0, ' ', 1, 'Arial, negrita, 8');
    list.PrintLn(espacios, list.Lineactual, leyenda, 2, 'Arial, negrita, 8');
    list.importe(75, list.lineactual,  '', totNettot, 3, 'Arial, negrita, 8');
    list.importe(85, list.lineactual, '', totOpexenta, 4, 'Arial, negrita, 8');
    list.importe(95, list.lineactual, '', totIva, 5, 'Arial, negrita, 8');
    list.importe(105, list.lineactual, '', totIvarec, 6, 'Arial, negrita, 8');
    list.importe(115, list.lineactual, '', totPercepcion, 7, 'Arial, negrita, 8');
    list.importe(125, list.lineactual, '', totPergan, 8, 'Arial, negrita, 8');
    list.importe(135, list.lineactual, '', totTotoper, 9, 'Arial, negrita, 8');
    list.importe(145, list.lineactual, '', totConnograv, 11, 'Arial, negrita, 8');
    list.importe(155, list.lineactual, '', totRetencion, 12, 'Arial, negrita, 8');
  ...
  if salida = 'X' then Begin
    if Trim(leyenda) = 'Subtotales:' then Begin
      ...
      Inc(fila); ff := Trim(IntToStr(fila));
      excel.setString('a' + ff, 'a' + ff, leyenda, 'Arial, negrita, 8');
      excel.setFormulaArray('d' + ff, 'd' + ff, '=suma(' + 'd' + IntToStr(ffi) + '..' + 'd' + IntToStr(f) + ')', 'Arial, negrita, 8');
      excel.setFormulaArray('e' + ff, 'e' + ff, '=suma(' + 'e' + IntToStr(ffi) + '..' + 'e' + IntToStr(f) + ')', 'Arial, negrita, 8');
      excel.setFormulaArray('f' + ff, 'f' + ff, '=suma(' + 'f' + IntToStr(ffi) + '..' + 'f' + IntToStr(f) + ')', 'Arial, negrita, 8');
      excel.setFormulaArray('g' + ff, 'g' + ff, '=suma(' + 'g' + IntToStr(ffi) + '..' + 'g' + IntToStr(f) + ')', 'Arial, negrita, 8');
      excel.setFormulaArray('h' + ff, 'h' + ff, '=suma(' + 'h' + IntToStr(ffi) + '..' + 'h' + IntToStr(f) + ')', 'Arial, negrita, 8');
      excel.setFormulaArray('i' + ff, 'i' + ff, '=suma(' + 'i' + IntToStr(ffi) + '..' + 'i' + IntToStr(f) + ')', 'Arial, negrita, 8');
...
begin
  ...
  while not tiva.EOF do
    begin
      ...
      if t_filtro = '6' then     // Prorrateo de combustibles
        if (tiva.FieldByName('ferecep').AsString >= p1) and (tiva.FieldByName('ferecep').AsString <= p2) and (tiva.FieldByName('idcom').AsString = p3) then LineaCombustibles(salida);
     tiva.Next;
    end;
...
procedure TTIvacompra.RegistrarCombustible(xidcompr, xtipo, xsucursal, xnumero, xcuit, xidcom: string; xlitros, xitc, xtvg, xiva3337: Real);
// Objetivo...: Registrar combustibles
begin
  if Buscar(xidcompr, xtipo, xsucursal, xnumero, xcuit) then Begin
    tiva.Edit;
    tiva.FieldByName('idcom').AsString  := xidcom;
    tiva.FieldByName('litros').AsFloat  := xlitros;
    tiva.FieldByName('itc').AsFloat     := xitc;
    tiva.FieldByName('tvg').AsFloat     := xtvg;
    tiva.FieldByName('iva3337').AsFloat := xiva3337;
    try
      tiva.Post
     except
      tiva.Cancel
...
procedure TTIvacompra.getDatosCombustible(xidcompr, xtipo, xsucursal, xnumero, xcuit: string);
Begin
  if Buscar(xidcompr, xtipo, xsucursal, xnumero, xcuit) then Begin
    idcomb  := tiva.FieldByName('idcom').AsString;
    litros  := tiva.FieldByName('litros').AsFloat;
    itc     := tiva.FieldByName('itc').AsFloat;
    tvg     := tiva.FieldByName('tvg').AsFloat;
    iva3337 := tiva.FieldByName('iva3337').AsFloat;
  end else Begin
    litros := 0; itc := 0; tvg := 0; iva3337 := 0; idcomb := '';
  end;
end;

procedure TTIvaCompra.LineaCombustibles(salida: char);
// Objetivo...: Imprimir una Línea de Detalle
begin
  if (salida = 'P') or (salida = 'I') then Begin
   if not infresumido then Begin
    ...
    list.importe(75,  list.lineactual, '', tiva.FieldByName('litros').AsFloat, 5, 'Arial, normal, 8');
    list.importe(85,  list.lineactual, '', tiva.FieldByName('Nettot').AsFloat, 6, 'Arial, normal, 8');
    list.importe(95,  list.lineactual, '', tiva.FieldByName('Iva').AsFloat, 7, 'Arial, normal, 8');
    list.importe(105, list.lineactual, '', tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat, 8, 'Arial, normal, 8');
    list.importe(115, list.lineactual, '', tiva.FieldByName('itc').AsFloat, 9, 'Arial, normal, 8');
    list.importe(125, list.lineactual, '', tiva.FieldByName('tvg').AsFloat, 10, 'Arial, normal, 8');
    list.importe(135, list.lineactual, '', tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat, 11, 'Arial, normal, 8');
    list.importe(145, list.lineactual, '', tiva.FieldByName('iva3337').AsFloat, 12, 'Arial, normal, 8');
    list.importe(155, list.lineactual, '', tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat + tiva.FieldByName('iva3337').AsFloat, 13, 'Arial, normal, 8');
    list.Linea(156, list.lineactual, ' ', 14, 'Arial, normal, 8', salida, 'S');
   end;
  end;
  if salida = 'T' then Begin
    nombre := tiva.FieldByName('rsocial').AsString + utiles.espacios(40 - Length(Trim(tiva.FieldByName('rsocial').AsString)));
    list.LineaTxt(utiles.sFormatoFecha(tiva.FieldByName('fecha').AsString) + ' ' + tiva.FieldByName('tipo').AsString + ' ' + tiva.FieldByName('sucursal').AsString + '-' + tiva.FieldByName('numero').AsString + ' ' + nombre + ' ', False);
    list.importeTxt(tiva.FieldByName('litros').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('nettot').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('iva').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('itc').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('tvg').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('iva3337').AsFloat, 12, 2, False);
    list.importeTxt(tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat + tiva.FieldByName('iva3337').AsFloat, 12, 2, True);
    Inc(lineasimpresas);
  end;

  //Subtotales
  totNettot     := totNettot     + tiva.FieldByName('litros').AsFloat;
  totOpexenta   := totOpexenta   + tiva.FieldByName('nettot').AsFloat;
  totIva        := totIva        + tiva.FieldByName('Iva').AsFloat;
  totIvarec     := totIvarec     + (tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat);
  totPercepcion := totPercepcion + tiva.FieldByName('itc').AsFloat;
  totPergan     := totPergan     + tiva.FieldByName('tvg').AsFloat;
  totTotoper    := totTotoper    + (tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat);
  totConnograv  := totConnograv  + tiva.FieldByName('iva3337').AsFloat;
  totRetencion  := totRetencion  + (tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat + tiva.FieldByName('iva3337').AsFloat);
  ...
  if salida = 'X' then Begin
    ...
    excel.setString('b' + ff, 'b' + ff, tiva.FieldByName('idcompr').AsString + ' ' + tiva.FieldByName('tipo').AsString + ' ' + tiva.FieldByName('sucursal').AsString + '-' + tiva.FieldByName('numero').AsString, 'Arial, normal, 8');
    excel.setString('c' + ff, 'c' + ff, Copy(tiva.FieldByName('rsocial').AsString, 1, 25), 'Arial, normal, 8');
    excel.setReal('d' + ff, 'd' + ff, tiva.FieldByName('litros').AsFloat, 'Arial, normal, 8');
    excel.setReal('e' + ff, 'e' + ff, tiva.FieldByName('nettot').AsFloat, 'Arial, normal, 8');
    excel.setReal('f' + ff, 'f' + ff, tiva.FieldByName('iva').AsFloat, 'Arial, normal, 8');
    excel.setReal('g' + ff, 'g' + ff, tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat, 'Arial, normal, 8');
    excel.setReal('h' + ff, 'h' + ff, tiva.FieldByName('itc').AsFloat, 'Arial, normal, 8');
    excel.setReal('i' + ff, 'i' + ff, tiva.FieldByName('tvg').AsFloat, 'Arial, normal, 8');
    excel.setReal('j' + ff, 'j' + ff, tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat, 'Arial, normal, 8');
    excel.setReal('k' + ff, 'k' + ff, tiva.FieldByName('iva3337').AsFloat, 'Arial, normal, 8');
    excel.setReal('l' + ff, 'l' + ff, tiva.FieldByName('Nettot').AsFloat + tiva.FieldByName('iva').AsFloat + tiva.FieldByName('itc').AsFloat + tiva.FieldByName('tvg').AsFloat + tiva.FieldByName('iva3337').AsFloat, 'Arial, normal, 8');
  end;
end;
...
begin
  ...
  if not datosdb.verificarSiExisteCampo('ivacompr', 'retencion', path) then datosdb.tranSQL(path, 'alter table ivacompr add retencion numeric');
  if not datosdb.verificarSiExisteCampo('ivacompr', 'importe', path) then datosdb.tranSQL(path, 'alter table ivacompr add importe numeric');
  if not datosdb.verificarSiExisteCampo('ivacompr', 'litros', path) then datosdb.tranSQL(path, 'alter table ivacompr add litros numeric');
  if not datosdb.verificarSiExisteCampo('ivacompr', 'itc', path) then datosdb.tranSQL(path, 'alter table ivacompr add itc numeric');
  if not datosdb.verificarSiExisteCampo('ivacompr', 'tvg', path) then datosdb.tranSQL(path, 'alter table ivacompr add tvg numeric');
  if not datosdb.verificarSiExisteCampo('ivacompr', 'iva3337', path) then datosdb.tranSQL(path, 'alter table ivacompr add iva3337 numeric');
  if not datosdb.verificarSiExisteCampo('ivacompr', 'idcom', path) then datosdb.tranSQL(path, 'alter table ivacompr add idcom varchar(3)');
